...
# --------------------------
# Memorizzazione livelli menu
# --------------------------
user_menu_level = {}  # chat_id -> livello precedente
...
def show_plan_info(update, context, plan, chat_id=None):
    # messaggio informativo
    if plan == "free":
        text = f"🆓 **Free Plan:** puoi fare fino a 5 pronostici al giorno, massimo 5 partite per pronostico!"
    elif plan == "2eur":
    ...
    # Salvo il livello corrente per il tasto Indietro
    if chat_id is not None:
        user_menu_level[chat_id] = "main_menu"
...
def show_alphabet_keyboard(plan, type_, chat_id=None, back_level=None):
    keyboard = [[{"text": c, "callback_data": f"filter_{type_}_{c}_{plan}"}] for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ"]
    # tasto indietro
    if back_level:
        keyboard.append([{"text": "🔙 Indietro", "callback_data": back_level}])
    keyboard.append([{"text": "🏟️ Menù principale ⚽", "callback_data": "main_menu"}])

    tipo_testo = "campionato" if type_ == "league" else "nazionale"
    if chat_id:
        user_menu_level[chat_id] = back_level or "main_menu"
    return {"text": f"🔍 Filtra per lettera ({tipo_testo}):", "reply_markup": {"inline_keyboard": keyboard}}

def show_filtered_options(type_, letter, plan, chat_id=None):
    ...
    if not filtered:
        return {"text": f"😕 Nessun {type_} trovato con la lettera '{letter}'.",
                "reply_markup": {"inline_keyboard": [[{"text": "🔙 Indietro", "callback_data": user_menu_level.get(chat_id, "main_menu")}]]}}
    ...
    keyboard.append([{"text": "🏟️ Menù principale ⚽", "callback_data": "main_menu"}])
    user_menu_level[chat_id] = f"select_{type_}_{plan}"
    tipo_testo = "campionato" if type_ == "league" else "nazionale"
...
def show_matches(update, context, league_id, plan, chat_id=None):
    ...
    keyboard.append([{"text": "🔙 Indietro", "callback_data": user_menu_level.get(chat_id, "main_menu")}])
    keyboard.append([{"text": "🏟️ Menù principale ⚽", "callback_data": "main_menu"}])
    user_menu_level[chat_id] = user_menu_level.get(chat_id, "main_menu")
    return {
...
def search_team_prompt(plan, chat_id=None):
    if chat_id:
        user_menu_level[chat_id] = f"plan_{plan}"
    return {"text": "🔎 Scrivi il nome della squadra che vuoi cercare:", "reply_markup": None}

def show_search_results(query, plan, chat_id=None):
